#!/usr/bin/env python
# Script para crear usuario demo nelson
import sys
import os
...
try:
    ...
    # 1. Obtener rol admin/administrador
    cur.execute("SELECT id_rol FROM roles WHERE LOWER(nombre_rol) LIKE '%admin%' LIMIT 1")
    ...
    print(f"[OK] Rol admin encontrado: id={admin_rol_id}")
    
    # 2. Obtener o crear programa
    ...
    if not prog:
        ...
        prog = cur.fetchone()
        conn.commit()
        print(f"[OK] Programa creado: id={prog['id_programa']}")
    else:
        print(f"[OK] Programa encontrado: id={prog['id_programa']}")
    prog_id = prog['id_programa']
    
    # 3. Eliminar usuario nelson anterior si existe
    cur.execute("DELETE FROM usuarios WHERE LOWER(nombre) = %s", ("nelson",))
    conn.commit()
    print("[OK] Usuario nelson anterior eliminado (si existía)")
    
    # 4. Crear nuevo usuario nelson
    ...
    conn.commit()
    print("[OK] Usuario nelson creado con contraseña: 1234")
